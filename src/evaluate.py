...
def normalize_text(s: str) -> str:
    ...
    s = s.lower()
    s = re.sub(r"[’‘']", " ", s)  # apostrophes (typographiques et ASCII) -> espace, avant la ponctuation
    s = "".join(ch for ch in s if ch not in string.punctuation)
    s = re.sub(r"\b(le|la|les|un|une|des|de|du|l|d)\b", " ", s)  # articles + élisions françaises
    s = " ".join(s.split())
    return s
